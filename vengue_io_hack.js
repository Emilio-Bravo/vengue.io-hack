...
var Hack = function () {
  ...
  this.setupHooks = function () {
    this.waitForProp("Movement")
      .then(this.hookMovement)
      .catch((e) => console.log(e));
    this.waitForProp("NetworkManager")
      .then(this.hookNetwork)
      .catch((e) => console.log(e));
    this.waitForProp("VengeGuard")
      .then(this.hookAnticheat)
      .catch((e) => console.log(e));
    this.waitForProp("Label")
      .then(this.hookLabel)
      .catch((e) => console.log(e));
  };
    ...
    window.addEventListener("keydown", (e) => {
      switch (e.keyCode) {
        case 57: // 9
          ...
          this.hooks.network.app.fire(
            "Chat:Message",
            "Los hacks",
            "Matar al reaparecer - " +
              (this.settings.autoKill ? "Activado" : "Desactivado"),
            !0
          );
          break;
        case 56: // 8
          ...
          this.hooks.network.app.fire(
            ...
            "Los Hacks",
            "Munición infinita - " +
              (this.settings.infAmmo ? "Activado" : "Desactivado"),
            !0
          );
          break;
        case 55: // 7
          ...
          this.hooks.network.app.fire(
            "Chat:Message",
            "Los Hacks",
            "Aimbot - " + (this.settings.aimbot ? "Activado" : "Desactivado"),
            !0
          );
          break;
        case 54: // 6
          ...
          this.hooks.network.app.fire(
            ...
            "Los Hacks",
            "Saltos infinitos - " +
              (this.settings.infJump ? "Activado" : "Desactivado"),
            !0
          );
          break;
        case 53: // 5
        ...
        case 122: // z
          ...
          this.hooks.network.app.fire(
            "Chat:Message",
            "Los Hacks",
            "ESP - " + (this.settings.esp ? "Activado" : "Desactivado"),
            !0
          );
          break;
        case 88: // x
          ...
          this.hooks.network.app.fire(
            "Chat:Message",
            "Los Hacks",
            "Escala de tiempo - " + (this.settings.timeScale + 1) + "x",
            !0
          );
          break;
        default:
  ...
  this.hookLabel = function () {
    Label.prototype.update = function (t) {
      if (!pc.isSpectator) {
        ...
        if (
          Date.now() - this.player.lastDamage > 1800 &&
          !FakeGuard.settings.esp
        )
  ...
  this.onTick = function () {
    if (FakeGuard.settings.aimbot) {
      ...
      for (var i = 0; i < players.length; i++) {
        ...
        let calcDist = Math.sqrt(
          (target.position.y - t.y) ** 2 +
            (target.position.x - t.x) ** 2 +
            (target.position.z - t.z) ** 2
        );
        if (calcDist < rec || !rec) {
          closest = target;
          rec = calcDist;
      ...
      let rayCastList = pc.app.systems.rigidbody
        .raycastAll(
          FakeGuard.hooks.movement.entity.getPosition(),
          FakeGuard.closestp.getPosition()
        )
        .map((x) => x.entity.tags._list.toString());
      let rayCastCheck =
        rayCastList.length === 1 && rayCastList[0] === "Player";
      if (closest && rayCastCheck) {
        ...
        FakeGuard.hooks.movement.lookX =
          e * 57.29577951308232 + Math.random() / 10 - Math.random() / 10;
        FakeGuard.hooks.movement.lookY =
          -1 *
          this.getXDire(
            closest.position.x,
            closest.position.y,
        ...
        FakeGuard.hooks.movement.setShooting(
          FakeGuard.hooks.movement.lastDelta
        );
      } else {
